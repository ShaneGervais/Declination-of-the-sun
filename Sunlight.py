...
delta = -np.arcsin(np.sin(epsilon) * np.cos(omega*(N+10) + eccentricity*omega*(365.24/math.pi)*np.sin(omega*(N-2))))
...
#labels
plt.xlabel("Days") #from one winter soltice to the other
